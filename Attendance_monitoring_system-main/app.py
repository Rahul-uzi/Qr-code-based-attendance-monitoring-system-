...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'f0eab79e87d4b0661db68262ef26f8b088d62a1e44a579f3'  
app.secret_key = 'f0eab79e87d4b0661db68262ef26f8b088d62a1e44a579f3' #Add your secret key here  
db = mysql.connector.connect(
    host='localhost',
    user='root', #add your username here
    password='',#add your password here
    database='database' # add your db name here
)
...
@app.route('/admin/download_attendance_pdf')
def download_attendance_pdf():
    class_sec = request.args.get('class_sec')
    ...
    c.drawString(100, 730, f"Class Section: {class_sec}")
    c.drawString(100, 710, f"Class Name: {class_name}")
    c.drawString(100, 690, f"Class Date: {class_date}")
    c.drawString(100, 670, "Student ID    Student Name    Attendance Status")
    ...
    cur.execute("""
        ...
        WHERE c.class_sec = %s AND c.class_name = %s AND c.class_date = %s
    """, (class_sec, class_name, class_date))
...
@app.route('/admin/download_attendance_excel')
def download_attendance_excel():
    ...
    df = pd.DataFrame(attendance_data, columns=["Student ID", "Student Name", "Attendance Status"])
...
def get_classes():
    ...
    # Fetch classes from the database based on the section
    cur.execute("SELECT class_id, class_name, class_date FROM classes WHERE class_sec = %s", (section,))
    classes = cur.fetchall()
    
    # Prepare the classes data to return as JSON
    class_list = []
